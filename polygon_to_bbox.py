                ...
                for object0 in coco_data.imgToAnns[img['id']]:
                    ...
                    for segment in segments:
                        ...
                        try:
                            ...
                            # print("main: {}".format(label_pixel))
                            # anno_txt.append(label)
                            if plot: 
                                polygon_coords[-1].append(np.vstack((label_pixel[1:].reshape(-1, 2), label_pixel[1:3])))
                                # print("label_pixel[1:]: {}".format(label_pixel[1:]))
                                # print("label_pixel[1:3]: {}".format(label_pixel[1:3]))
                                segment_coords[-1].append(segment)
                        except Exception as e:
                            print('Warning: Ignore label, ', e)

                    print("polygon_coords: {}".format(polygon_coords[-1]))
                    print('-----------')
                if plot: 
...
def normalize_anchors(label, img_h, img_w):
    ...
    # print("normalize: {}".format(label_pixel))
...
seg2poly('./yolov5_polygon/coco20', plot=True)